...
target_level = st.sidebar.selectbox(
    "選擇對照標竿年級 / 語體",
    ["高中五年級/高二 (Ting 2024)", "學測優秀作文 (GSAT)", "真實學術論文 (RA)"]
)
...
# 文字輸入區
user_text = st.text_area("請貼入英文課文或教材文本：", height=150, value="The English textbooks used in senior high schools contain complex syntactic structures that challenge students in their learning process.")

if st.button("🚀 開始多維度診斷", type="primary"):
    # 進行運算
    ...
    col4.metric("工作記憶超載點", f"{len(mdd_res['overload_spans'])} 處")
    ...
    with tab1:
        ...
        if l2sca_res["CNP/C"] < current_norm["CNP/C"]:
            st.info(f"ℹ️ **片語凝聚度提醒**：當前文本 CNP/C 為 {l2sca_res['CNP/C']}，低於標竿值 ({current_norm['CNP/C']})。若目標為銜接大學閱讀，建議增加名詞片語後置修飾。")

    with tab2:
        st.subheader("結構與數值詳細分析")
        fig = go.Figure(data=[
            go.Bar(name='當前文本', x=['MLS', 'CNP/C', 'MDD'], y=[l2sca_res['MLS'], l2sca_res['CNP/C'], mdd_res['mdd']]),
            go.Bar(name='標竿基準', x=['MLS', 'CNP/C', 'MDD'], y=[current_norm['MLS'], current_norm['CNP/C'], current_norm['MDD_target']])
        ])
        fig.update_layout(barmode='group', title_text='當前文本 vs 標竿常模指標對比')
